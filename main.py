...
def login():
    ...
    if row: 
        ...
        # Allocate amount of points for participant or attendee
        ...
        # set a textbox to a specific value
        ...
        # enables searching for specific person in table
        def search_by_name():
        ...
        # (Not Functional Yet) Get quarter column of students
        # def get_quarter_students():
        #     quarter_value = quarter_spinbox.get()
        #     items = db.find_quarter_students(quarter_value)
        #     return items
        ...
        # Instructions for form sub-menu option
        ...
        # Expore student records button to get CSV file
        ...
        # Two tables with varying information
...
# Username field
info_user = tkinter.Label(window,text = 'Username: ')
info_user.grid(row =1, column =0)
userinput = tkinter.Entry(window,textvariable=user_input)
...
# Password Field
info_pass = tkinter.Label(window, text = 'Password: ')
info_pass.grid(row=3, column =0)
passinput = tkinter.Entry(window,textvariable = pass_input, show = "*")
...
# Login button
login_btn = tkinter.Button(window, text="Login",command=login)
